# ============================================================
# MobileNetV2 + GELU + CrossEntropy Comparative Study (PyTorch)
...
def build_model():
    ...
    # FIXED: Removed Softmax output because CrossEntropyLoss acts on raw logit scores
...
def train_dataset(name, root):
    ...
    with torch.no_grad():
        for images, labels in test_loader:
            ...
            with autocast(device_type="cuda"):
                outputs = model(images)
                # FIXED: Apply Softmax explicitly HERE during evaluation to extract true probabilities
    ...
    # Return additional raw collections to use for live visual graphing later
    return metrics, history, {"y_true": y_true, "y_prob": y_prob, "cm": cm}


# ---------------- RUN PIPELINE ----------------
# ---------------- RUN PIPELINE ----------------
# Wrap this entire bottom block inside the main check to protect Windows worker processes
if __name__ == '__main__':
    ...
    plt.title('Training and Validation Loss Comparison (GELU)', fontsize=13, fontweight='bold')
    plt.xlabel('Epochs')
    ...
    plt.title('Training and Validation Accuracy Comparison (GELU)', fontsize=13, fontweight='bold')
    plt.xlabel('Epochs')
    ...
    plt.bar(x_idx - width/2, filtered_comp['140K'], width, label='140K Dataset (GELU)', color='royalblue')
    plt.bar(x_idx + width/2, filtered_comp['200K'], width, label='200K Dataset (GELU)', color='darkorange')

    plt.title('Final Core Evaluation Benchmarks (GELU Model)', fontsize=13, fontweight='bold', pad=15)
